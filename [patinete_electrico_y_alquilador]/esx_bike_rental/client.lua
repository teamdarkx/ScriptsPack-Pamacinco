...
function OpenBikesMenu()
	...
	if data.current.value == 'bike2' then
		if Config.EnablePrice then
			TriggerServerEvent("esx:bike:lowmoney", Config.PriceServ_electricscooter) 
			TriggerEvent("chatMessage", _U('bikes'), {255,0,255}, _U('bike_pay', Config.PriceServ_electricscooter))
		end
		
		if Config.EnableEffects then
			spawn_effect("Serv_electricscooter")
		else
