...
AddEventHandler('vender:esmicoche',function(matr,id,price,car,name)
	...
	if data[1] then
		for i=1,#data,1 do
			...
			if tostring(vehicle.plate) == tostring(matr) then
				var = true
				break
			end
		end
...
TriggerEvent('es:addCommand', 'vendercoche', function(source, args, user)
	local source = source
	if #args == 0 then
		TriggerClientEvent('chatMessage', source, "Vendedor", {255, 0, 0}, "Debes poner una ID!")
	else
		if GetPlayerName(tostring(args[1])) then--and tonumber(args[2]) ~= source then
			if #args == 1 then TriggerClientEvent('chatMessage', source, "Vendedor", {255, 0, 0}, "Debes poner un precio!") return 0 end
