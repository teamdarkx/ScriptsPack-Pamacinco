...
AddEventHandler('playerSpawned',function()
	...
	ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(banned)
		for i=1,#banned,1 do
			modelosbanned[#modelosbanned+1] = banned[i]
		end
	end)
end)
...
AddEventHandler('vender:compruebocoche',function(id,precio)
	...
	--	if GetDistanceBetweenCoords(coords1,coords2) < 1.5 then
			if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then 	
				...
				local matr =  trim(GetVehicleNumberPlateText(vehicle))
